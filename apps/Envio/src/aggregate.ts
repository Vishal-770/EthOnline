...
class OnChainAggregator {
  ...
  analyzeOnChain(): OnChainMetrics {
    console.log(`📊 Analyzing ${this.transactions.length} on-chain transactions...`);
    ...
    const whaleVolume = txValues.filter(v => v > whaleThreshold).reduce((sum, v) => sum + v, 0);
    const largestTransaction = Math.max(...txValues, 0);

    const txPerDay = this.transactions.length / Math.max(1, (now - this.transactions[0]?.timestamp) / day);
    ...
    const { holderConcentration, giniCoefficient, top10HoldersVolume } = this.analyzeDistribution();
  ...
  private analyzeBuySellPressure(): { buyVsSellRatio: number; avgBuySize: number; avgSellSize: number } {
    let buyVolume = 0, sellVolume = 0, buyCount = 0, sellCount = 0;

    this.transactions.forEach(tx => {
      if (!tx.eventData?.value || !tx.eventData?.from || !tx.eventData?.to) return;
      ...
      const isDexAddress = (addr: string) => 
        addr.includes('uniswap') || addr.includes('sushiswap') || 
        addr.includes('pancakeswap') || addr.length < 42;
    ...
    return {
      buyVsSellRatio: sellVolume > 0 ? buyVolume / sellVolume : buyVolume > 0 ? 999 : 1,
      avgBuySize: buyCount > 0 ? buyVolume / buyCount : 0,
      avgSellSize: sellCount > 0 ? sellVolume / sellCount : 0
    };
  }

  private analyzeDistribution(): { holderConcentration: number; giniCoefficient: number; top10HoldersVolume: number } {
    const holdings = new Map<string, number>();

    this.transactions.forEach(tx => {
      if (!tx.eventData?.from || !tx.eventData?.to || !tx.eventData?.value) return;
    ...
    const holderConcentration = totalBalance > 0 ? (top10Volume / totalBalance) * 100 : 0;
  ...
  private analyzeTimePatterns(): { peakActivityHour: number; mostActiveDay: string } {
  ...
  private detectTrend(recent: Transaction[], older: Transaction[]): 'increasing' | 'decreasing' | 'stable' {
  ...
  private detectRisks(): { suspiciousPatterns: string[]; riskScore: number } {
    ...
    const prev24h = this.transactions.filter(tx => 
      tx.timestamp > now - 172800 && tx.timestamp <= now - 86400
    );
  ...
  generateCombinedAnalysis(onChainMetrics: OnChainMetrics): CombinedAnalysis {
    ...
    else if (adjustedScore > 75 && onChainMetrics.momentumScore > 20) investmentSignal = 'strong_buy';
    ...
    if (onChainMetrics.momentumScore > 30) {
      keyFindings.push(`Strong positive momentum: +${onChainMetrics.momentumScore.toFixed(1)}%`);
    }
    if (onChainMetrics.buyVsSellRatio > 2) {
      opportunities.push(`High buy pressure: ${onChainMetrics.buyVsSellRatio.toFixed(2)}x more buys than sells`);
    }
    if (onChainMetrics.holderConcentration > 60) {
      warnings.push(`High concentration: Top 10 holders own ${onChainMetrics.holderConcentration.toFixed(1)}% of supply`);
    }
...
async function main() {
  ...
  const transactionData = JSON.parse(fs.readFileSync('./token_transactions.json', 'utf8')); // Your 5000+ transactions
  const tokenData = JSON.parse(fs.readFileSync('./token_analysis.json', 'utf8')); // Your token market data
