    ...
    try {
      ...
      if (Array.isArray(metaRaw) && metaRaw.length > 0) {
        ...
        if (typeof metaRaw.address === 'string' && metaRaw.address.includes(',')) {
          ...
          const remaining = addresses.filter((a: string) => a !== singleMeta.address);
      ...
      if (!formattedTokens || formattedTokens.length === 0) {
        formattedTokens = addresses.slice(0, 10).map((addr: string, i: number) => ({
          address: addr,
          symbol: `TKN${i + 1}`,
          liquidity: 'N/A',
          change: '0.00%',
